...
export default function NutriAIPro() {
  ...
  useEffect(() => {
    const init = async () => {
        // Se c'è un token iniziale (ambiente di test), usalo. Altrimenti attendi azione utente o anonimo.
        if (typeof __initial_auth_token !== 'undefined' && __initial_auth_token) {
            await signInWithCustomToken(auth, __initial_auth_token);
        } else {
             // Opzionale: decommenta se vuoi login anonimo automatico, altrimenti mostra login screen
    ...
    const loadData = async () => {
      ...
      try {
        ...
        if (keySnap.exists() && keySnap.data().gemini_key) {
          ...
          // Fallback per test immediato se necessario (lasciare vuoto in produzione)
          setApiKey(""); 
        }
      } catch (e) {
...
// --- SUB COMPONENTS (Aggiornati per accettare apiKey) ---
    ...
    return (
      <div className="p-5 min-h-screen bg-white animate-in slide-in-from-bottom-5">
        ...
        <div className="space-y-6">
          {/* Semplificato per brevità, stessa logica di prima */}
          <div>
